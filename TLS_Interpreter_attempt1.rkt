...
(define self-evaluating?
  (lambda (exp) (or (number? exp) (boolean? exp) (string? exp))))
...
(define cond?
  (lambda (exp)
    (and (pair? exp) (eqv? (car exp) 'cond))
    ))

(define text-of-quotation cadr)
(define parameters-of cadr)
...
(define *self  (lambda (e table) e))
(define *quote (lambda (e table) (text-of-quotation e)))

(define *identifier (lambda (e table) (table e)))
...
;; ===========================================================================
; Table Primitives that are needed. In TLS, env's are referred to as tables. So following TLS,
; we use table/tables as the names and not env. 
;; ===========================================================================
(define empty-table
  (lambda (sym)
...
;; extend-table : (listof symbol) (listof value) table -> table
(define (extend-table names vals older)
  (if (null? names)
      older
      (lambda (sym)
        (let loop
          ((ns names) (vs vals))
          (cond ((null? ns) (older sym))
                ((eqv? sym (car ns)) (car vs))
                (else
                 (loop (cdr ns) (cdr vs))))))))

(define primitive-names
  '(add1 sub1 zero? + - * cons car cdr null? pair? number? symbol?))

(define primitive-vals
  (list add1 sub1 zero? + - * cons car cdr null? pair? number? symbol?))
...
(define initial-table
  (extend-table primitive-names primitive-vals empty-table))


;Action for lambda
(define (*lambda e table)
  (let ((formals (parameters-of e))
        (body    (body-of e)))
    (lambda vals
      (meaning body
               (extend-table formals vals table)))))
...
;Action for cond
(define truthy? (lambda (v) (not (eqv? v #f))))

(define (*cond e table)
  (let loop ((clauses (cdr e)))
    (let* ((clause (car clauses))
           (test   (car clause)))
      (cond ((eqv? test 'else) (meaning (cadr clause) table))
            ((truthy? (meaning test table)) (meaning (cadr clause) table))
...
(define (*application e table)
  (apply (meaning (function-of e) table)
         (map (lambda (arg) (meaning arg table))
              (arguments-of e))))
...
(define (expression-to-action exp)
  (cond
    ((self-evaluating? exp) *self)
    ((quoted? exp) *quote)
    ((lambda? exp) *lambda)
    ((cond? exp) *cond)
    ((symbol? exp) *identifier)
    (else
     *application)))

(define (meaning exp table)
  ((expression-to-action exp) exp table))

;; External entry point expected by the test scripts
(define (value exp)
  (meaning exp initial-table))
...
;TESTING - Code Provided by Professor on MS Teams
(value '((lambda (x) (add1 x)) 3)) ;returns 4

(value '((lambda (x) (add1 x))
	 ((lambda (x) (add1 x)) 4))) ;returns 6
